...
CVRF_DEFAULT_NAMESPACES = {
    part.upper(): '{{{CVRF_OASIS_ROOT}v{CRVF_DEFAULT_SEMANTIC_VERSION}/{part}}}' for part in CVRF_PARTS
}

CVRF_DEFAULT_CATALOG = pathlib.Path(
    'csaf_lint', 'schema', f'catalog_{CRVF_DEFAULT_SEMANTIC_VERSION.replace(".", "_")}.xml'
)
...
CVRF_PRE_OASIS_NAMESPACES = {
    part.upper(): '{{{CVRF_OASIS_ROOT}v{CRVF_DEFAULT_SEMANTIC_VERSION}/{part}}}' for part in CVRF_PARTS
}

CVRF_PRE_OASIS_CATALOG = pathlib.Path(
    'csaf_lint', 'schema', f'catalog_{CRVF_PRE_OASIS_SEMANTIC_VERSION.replace(".", "_")}.xml'
)
...
def validate_json(document, schema, conformance=None) -> typing.Tuple[int, str]:
    ...
    LOG.debug(
        f'caller site json validation list(document.keys())={list(document.keys())},'
...
def validate(document, schema, conformance=None) -> typing.Tuple[int, str]:
    ...
    LOG.debug(
        f'caller site validation: schema={schema}, catalog={catalog}, xml_tree={xml_tree},'
        f' request_version={request_version}'
    )
...
def load_xml(document_path):
    ...
    """
    try:
        cvrf_doc = etree.parse(document_path, etree.XMLParser(encoding=ENCODING))
...
def derive_version_from_namespace(root):
    ...
    for version, namespace in CVRF_VERSION_NS_MAP.items():
        LOG.debug(
            f'versions from namespace callee site naive trial str_rep_root={str_rep_root},'
            f' version={version}, namespace={namespace}'
        )
        if version in str_rep_root:
            LOG.debug(
                f'versions from namespace callee site naive match root={root},'
                f' version={version}, namespace={namespace}'
            )
            return version, namespace
        LOG.debug(
            f'versions from namespace callee site naive miss root={root},' f' version={version}, namespace={namespace}'
        )
...
def derive_schema_path(catalog, request_version, schema):
    ...
    if schema:
        LOG.debug(
            f'xml validate try reading schema catalog={catalog},'
            f" schema={schema}, catalog env=({os.getenv('XML_CATALOG_FILES')})"
        )
    else:
        LOG.debug(
            f'xml validate try reading local implicit schema catalog={catalog},'
            f" schema={schema}, catalog env=({os.getenv('XML_CATALOG_FILES')})"
        )
...
def xml_validate(schema, catalog, xml_tree, request_version):
    ...
    LOG.debug(
        f'xml validate parameters: schema={schema}, catalog={catalog},'
    ...
    try:
        with open(schema, 'r') as handle:
            LOG.debug(
                f'xml validate success reading schema catalog={catalog},'
                f" schema={schema}, catalog env=({os.getenv('XML_CATALOG_FILES')})"
            )
...
def main(argv=None, embedded=False, debug=None):
    ...
    LOG.debug(
        f'post dispatch embedded={embedded}, argv={argv}, num_args={num_args}, pos_args={pos_args},'
